...
📊 **Уровень:** [X]/10`;
        ...
        // Если ИИ ответил успешно, сохраняем его ответ
        userCache[chatId].statusText = result.response.text();
    } catch (err) {
        console.error("⚠️ Ошибка Gemini / Сети:", err.message);
        // РЕЗЕРВ: Если ИИ сломался, выдал ошибку или нет ключа — используем шаблон
    ...
    bot.editMessageText(`${userCache[chatId].statusText}\n\n📍 Теперь отправьте **геолокацию** этого места (через скрепку 📎)!`, {
        ...
        // Запасной вариант отправки, если Markdown выдаст ошибку
        bot.editMessageText(`${userCache[chatId].statusText}\n\n📍 Теперь отправьте геолокацию этого места (через скрепку 📎)!`, {
            chat_id: chatId,
            message_id: waitMsg.message_id
        });
...
bot.on('location', async (msg) => {
    ...
    try {
        // 1. Отправляем фото с описанием в рабочую группу
